...
export const projects: ProjectDetail[] = [
  ...
  {
    title: "Inventra — Inventory & Retail SaaS",
    description:
    ...
    type: "app",
    features: [
      "Multi-tenant SaaS: org subdomains, signup queue, staff approval, trial provisioning",
      "Stripe billing: Checkout, Customer Portal, webhooks, subscription gating (trial/active/past_due)",
      "Platform console (/platform): approvals, plans, tenant features, support desk, SaaS analytics",
      "Tenant portals: marketing site, admin, barcode scanner POS, and data-entry interfaces",
      "Multi-branch inventory, barcode scan-to-sell, wholesale B2B, POs, invoicing, and returns",
      "Org onboarding checklist, branding, custom domain DNS verification, operational analytics",
      "Telegram bot: alerts, EOD digests, inline approvals; optional SMTP fallback for EOD",
      "Support tickets with SLA, staff threads, escalations, and auto-assign routing",
      "SaaS KPIs: MRR/plan mix, trial→paid, cohort retention snapshots, CSV exports",
      "Security: dual JWT (tenant + SaaS staff), Argon2, tenant isolation e2e, webhook idempotency",
      "CI/CD: GitHub Actions build/test; automated deploy to production VPS on main",
    ],
    security: [
      ...
      "RBAC with branch-scoped roles (Admin, Branch Manager, Data Entry, Sales, Hybrid)",
      "Stripe webhook signature verification with idempotent event store and retry worker",
      "Helmet, CSP, rate limiting on auth and scan paths; Telegram webhook secrets",
      "Tenant isolation covered by e2e tests (cross-org reads return 404)",
      "Structured audit logs for SaaS approvals, billing, feature toggles, and staff actions",
    ],
    architecture:
      "Monorepo: inventory-api (NestJS REST, ~20 domain modules, Prisma/PostgreSQL), inventory-web (Next.js 16 marketing + tenant UIs + platform console), inventory-local (Docker Compose for Postgres, Redis, MinIO). Tenancy resolves org from subdomain or verified custom domain; scheduled jobs (@nestjs/schedule) for EOD, Telegram digests, Stripe webhook retries, and cohort snapshots. S3-compatible storage for product images. Production path: VPS + Caddy TLS, PM2, prisma migrate deploy, GitHub Actions deploy workflow.",
  },
  {
